...
describe('Summoner_services', () => {
    ...
    test('getLeaderBoard', async() => {
        ...
        const leaderboard = await getLeaderBoard(tier, division, queueType, region);
    ...
    test('updateDBLeaderboard', async() => {
        ...
        const updatedLeaderboard = await updateDBLeaderboard(tier, division, queueType, region);
        const expectedSize = 10;
        const expectedRank = 'Challenger I'
