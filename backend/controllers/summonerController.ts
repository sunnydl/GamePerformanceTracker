...
export const getSummonerInfoByName = async (req: Request, res: Response) => {
    ...
    try {
        const summonerDTO = (await summonerService.getSummonerByName(
            summonerName,
            region
        )) as SummonerDTO; // get the summonerInfo
        res.status(200).json(summonerDTO);
    } catch (err: any) {
        res.status(
            exceptionHandler.exceptionCodeHandler(err?.response?.status)
        ).send({
            message: err.message,
            error: err.toString(),
        });
...
export const getLeaderBoard = async (req: Request, res: Response) => {
    ...
    const region: string = (req.query.region as string) || 'NA'; // default as NA if no region input
    try {
        const leaderBoard = await summonerService.getLeaderBoard(
            tier,
            division,
            queueType,
            region
...
/**
 * Controller for updating and return the updated Leaderboard data from DB
 *
 * @param {Request} req HTTP request
 * @param {Response} res HTTP response
 * @return {Promise<void>}
 */
export const getUpdatedLeaderBoard = async (req: Request, res: Response) => {
    const tier: string = (req.query.tier as string) || 'Challenger'; // default challenger queue;
    const division: string = (req.query.division as string) || 'I'; // default as I
    const queueType: string = (req.query.queueType as string) || 'SOLO'; // default solo queue
    const region: string = (req.query.region as string) || 'NA'; // default as NA if no region input
    try {
        const leaderBoard = await summonerService.updateDBLeaderboard(
            tier,
            division,
            queueType,
            region
