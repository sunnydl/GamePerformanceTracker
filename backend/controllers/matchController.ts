...
export const getMatchHistory = async (req: Request, res: Response) => {
    ...
    try {
        const puuid: string = await summonerService.findSummonerPuuid(
            summonerName,
            region
        );
        const matchData = await matchService.getMatchHistoryData(
            puuid,
            region,
            matchType,
            numOfMatch
...
export const getMatchChartData = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const matchChartData: Array<MatchChartDataDTO> =
            await matchService.getMatchChartData(
                puuid,
                region,
                matchType,
                numOfMatch
 ...
 * @return {Promise<void>}
 */
export const getUpdatedMatchHistoryData = async (
    req: Request,
    res: Response
) => {
    const summonerName: string = req.query.summonerName as string;
    ...
    try {
        ...
        const matchData = await matchService.updateDBMatchHistoryData(
            puuid,
            region,
            matchType,
            numOfMatch
